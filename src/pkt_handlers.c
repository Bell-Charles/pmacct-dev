...
void NF_ingress_vrf_name_handler(struct channels_list_entry *chptr, struct packet_ptrs *pptrs, char **data)
{
  ...
  else vlen_prims_insert(pvlen, COUNT_INT_INGRESS_VRF_NAME, MAX_VRF_NAME_STR_LEN, (u_char *) pptrs->ingress_vrf_name, PM_MSG_STR_COPY);
...
void NF_egress_vrf_name_handler(struct channels_list_entry *chptr, struct packet_ptrs *pptrs, char **data)
{
  ...
  else vlen_prims_insert(pvlen, COUNT_INT_EGRESS_VRF_NAME, MAX_VRF_NAME_STR_LEN, (u_char *) pptrs->egress_vrf_name, PM_MSG_STR_COPY);
}
...
void NF_vrf_name_handler(struct channels_list_entry *chptr, struct packet_ptrs *pptrs, char **data)
{
  ...
  else vlen_prims_insert(pvlen, COUNT_INT_VRF_NAME, MAX_VRF_NAME_STR_LEN, (u_char *) pptrs->vrf_name, PM_MSG_STR_COPY);
...
void NF_cvlan_handler(struct channels_list_entry *chptr, struct packet_ptrs *pptrs, char **data)
{
  struct struct_header_v5 *hdr = (struct struct_header_v5 *) pptrs->f_header;
  struct template_cache_entry *tpl = (struct template_cache_entry *) pptrs->f_tpl;
  struct pkt_tunnel_primitives *ptun = (struct pkt_tunnel_primitives *) ((*data) + chptr->extras.off_pkt_tun_primitives);
...
void NF_peer_src_ip_handler(struct channels_list_entry *chptr, struct packet_ptrs *pptrs, char **data)
{
  ...
  case 9:
    ...
    if ((tpl->fld[NF9_EXPORTER_IPV4_ADDRESS].count || tpl->fld[NF9_EXPORTER_IPV6_ADDRESS].count)
	  ...
	  got_peer_src_ip = TRUE;
	}
      }

      if (!got_peer_src_ip && tpl->fld[NF9_EXPORTER_IPV6_ADDRESS].count) {
